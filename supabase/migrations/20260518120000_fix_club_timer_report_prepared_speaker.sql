-- Club timer meeting-wise rollup missed rows saved as prepared_speaker (app default).

CREATE OR REPLACE FUNCTION public._timer_report_is_prepared_category(p_category text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
COMMENT ON FUNCTION public._timer_report_is_prepared_category(text) IS
  'True when timer_reports.speech_category counts toward Prepared speeches on Club tab.';
  ...
  RETURN (
    ...
    timer_base AS (
      SELECT
        tr.meeting_id,
        tr.recorded_by,
        sum(CASE WHEN public._timer_report_is_prepared_category(tr.speech_category) THEN 1 ELSE 0 END) AS prepared_total,
        sum(CASE WHEN public._timer_report_is_prepared_category(tr.speech_category) AND tr.time_qualification THEN 1 ELSE 0 END) AS prepared_qualified,
