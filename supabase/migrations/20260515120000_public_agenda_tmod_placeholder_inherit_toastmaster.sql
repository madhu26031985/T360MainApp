/*
  Public agenda: TMOD placeholder sections ("TMOD continues with theme", "TMOD closing section")
  are not role-based rows, so they often have no assignee while Toastmaster of the Day is booked.
  Inherit the visible Toastmaster of the Day assignee name for those sections on public links.
*/
...
BEGIN
  ...
  v_theme := NULLIF(trim(COALESCE(v_meeting.theme, '')), '');
  IF v_theme IS NULL THEN
    SELECT NULLIF(trim(COALESCE(tmd.theme_of_the_day, '')), '')
    INTO v_theme
    FROM public.toastmaster_meeting_data tmd
    WHERE tmd.meeting_id = p_meeting_id
      AND tmd.club_id = v_meeting.club_id
      AND NULLIF(trim(COALESCE(tmd.theme_of_the_day, '')), '') IS NOT NULL
    ORDER BY tmd.updated_at DESC NULLS LAST
    LIMIT 1;
  END IF;
  ...
  FROM (
    SELECT
      ...
      jsonb_build_object(
        ...
        NULLIF(trim(
          COALESCE(
            NULLIF(trim(COALESCE(ap_assigned.full_name, mai.assigned_user_name, '')), ''),
            CASE
              WHEN trim(mai.section_name) IN ('TMOD continues with theme', 'TMOD closing section') THEN
                (
                  SELECT NULLIF(trim(COALESCE(ap_tm.full_name, mai_tm.assigned_user_name, '')), '')
                  FROM public.meeting_agenda_items mai_tm
                  LEFT JOIN public.app_user_profiles ap_tm ON ap_tm.id = mai_tm.assigned_user_id
                  WHERE mai_tm.meeting_id = mai.meeting_id
                    AND COALESCE(mai_tm.is_visible, true) = true
                    AND lower(trim(COALESCE(mai_tm.section_name, ''))) LIKE '%toastmaster%of%the%day%'
                  ORDER BY mai_tm.section_order NULLS LAST, mai_tm.id
                  LIMIT 1
                )
              ELSE NULL
            END
...
COMMENT ON FUNCTION public.get_public_meeting_agenda_by_club(uuid, text, uuid) IS
  'Public agenda: meeting+club+items; prefers profile full names; TMOD placeholder sections inherit Toastmaster of the Day assignee.';
